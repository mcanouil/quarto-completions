...
# The stamp sits above the line naming what the scripts were built from, which
# every generated file carries. Read with a looser pattern, that line is the
# one installed_stamp would match, and the advice below would report a commit
# where a version belongs.
SOURCED_HOME="$(scenario_home other-sourced)"
mkdir -p "${SOURCED_HOME}/.zfunc"
printf '%s\n%s\n%s\n%s\n' \
  "#compdef quarto" \
  "# Quarto CLI completions, generated by quarto-completions." \
  "# Quarto 0.1.0 (release channel)." \
  "# Built from quarto-dev/quarto-cli main@e3f1c0ab1234567890abcdef1234567890abcdef." \
  >"${SOURCED_HOME}/.zfunc/_quarto"
sourced_output="$(scenario_run "${SOURCED_HOME}" --shell bash)"
expect_contains "other shells: a stale file carrying a source line is still read" \
  "${sourced_output}" "Quarto 0.1.0, release channel"
