...
check "pwsh: completion script parses" pwsh pwsh -NoProfile -Command \
  "\$errors = \$null; [System.Management.Automation.Language.Parser]::ParseFile('${COMPLETIONS}/quarto.ps1', [ref]\$null, [ref]\$errors) | Out-Null; if (\$errors) { \$errors; exit 1 }"
...
check "install.ps1 passes PSScriptAnalyzer" pwsh pwsh -NoProfile -Command \
  "Invoke-ScriptAnalyzer -Path '${ROOT}/docs/install.ps1' -EnableExit -Severity Error,Warning"
