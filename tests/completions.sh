...
test_zsh() {
  ...
  # even under `zsh -f`, which is what the runs above use.
  candidates="$(zsh "${ROOT}/tests/zsh-complete.zsh" fpath "${COMPLETIONS}" "${SCRATCH}/zsh" 'quarto publish ' 2)"
  expect_missing "zsh: no group option among the providers" "${candidates}" "-default-"

  # The rest of them only appear once a matcher-list style is set, which no
  # `zsh -f` run has, so this one starts a shell that reads an rc file setting
  # the style Oh My Zsh ships.
